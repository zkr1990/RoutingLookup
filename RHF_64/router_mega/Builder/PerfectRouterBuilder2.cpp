...
//---------------------------------------------------------------------------
bool TPerfectRouterBuilder2::Build(int *pn, uint64_t *hash,int nHash,int nBucket,int nSlot)
// bool TPerfectRouterBuilder2::Build(int *pn, uint32_t *hash,int nHash,int nBucket,int nSlot, int sl0, int sl1, int s0, int s1)
{
	...
	for(int i=0;i<nSlot;i++)
	{
		...
		for(int d=1;d<maxD;d++)
		{
			...
			{
				//// for 48 bit adder in FPGA
				uint64_t x_key = sortedHash[startPos+j] >> 16; // low 11 bits to index Slot table
				// uint64_t d1 = (uint64_t) (d + d << 12 + ((uint64_t)d) << 24 + ((uint64_t)d) << 36);
				uint64_t d1 = (uint64_t) d;
				int f = ( ((x_key + d1) >> 8) << shiftBucket >> shiftBucket);

				// below all fail 
				// int f = ( (((x_key << 39 >> 39) * (uint64_t)d ^ (x_key >> 25)) >> 14) << shiftBucket >> shiftBucket);
				// uint64_t x_key1 = sortedHash[startPos+j];
				// x_key1 = x_key1 ^ (uint64_t)d;
				// x_key1 ^= (x_key1 >> 16);
				// x_key1 ^= (x_key1 >> 16);
				// x_key1 ^= (x_key1 >> 16);
				// int f = x_key1 << 48 >> 48;
				// int f = FastIntHash(x_key,d) << shiftBucket >> shiftBucket;
