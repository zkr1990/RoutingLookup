...
#include "PerfectRouterTester2.h"
...
bool TPerfectRouterTester2::Test(const char *funcFileName, uint64_t *hash, int *pn,int nHash)
{
    ...
    for(int i=0;i<nHash;i++)
	{
        ...
        // Step3: universal shift hash and mod m
        // uint64_t x_key = hash[i] >> 13;
        uint64_t x_key = sortedHash[startPos+j] >> 16; // 64 - 16 = 48
        int f = ( ((x_key + (uint64_t)d) >> 8) << shiftBucket >> shiftBucket);
        // Step4: Interface number lookup
        if (TbucketArray[f] != pn[i])
        {
			fprintf(stderr,"###############################################  \r\n");
