...
bool TPerfectRouterTester1::Test(const char *funcFileName, uint64_t *hash, int *pn,int nHash)
{
    ...
    for(int i=0;i<nHash;i++)
	{
        ...
        // int f = ( (((x_key << 39 >> 39) * (uint64_t)d + (x_key >> 25)) >> 15) << shiftBucket >> shiftBucket);
