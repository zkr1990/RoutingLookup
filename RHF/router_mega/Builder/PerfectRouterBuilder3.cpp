...
//---------------------------------------------------------------------------
bool TPerfectRouterBuilder3::Build(int *pn, uint32_t *hash,int nHash,int nBucket,int nSlot)
{
	if(nSlot==0)
		return false;
	...
	for(int i=0;i<nSlot;i++)
	{
		...
		for(int d=1;d<maxD;d++)
		{
			...
			{
				uint32_t x_key = sortedHash[startPos+j] >> 11;

				int f = ( ((x_key * (uint32_t)d) >> 7) << shiftBucket >> shiftBucket);
